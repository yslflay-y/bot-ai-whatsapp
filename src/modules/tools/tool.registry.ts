...
import { ToolDefinition, ToolExecutionContext, isToolAuthorized } from "./tool.interface.js";
...
export class ToolRegistry {
   ...
   */
  getOpenAITools(userRole: Role = Role.USER) {
    return this.getAllTools()
      ...
      .map((tool) => ({
        ...
        function: {
          ...
          parameters: (tool.schema as any)._def ? this.zodToJsonSchema(tool.schema) : {},
        },
      }));
  }
   ...
   */
  getGeminiFunctionDeclarations(userRole: Role = Role.USER) {
    return this.getAllTools()
      .filter((tool) => isToolAuthorized(tool, userRole))
      .map((tool) => ({
        name: tool.name,
  ...
  async executeTool(
    ...
    if (!isToolAuthorized(tool, context.user.role)) {
      throw new AuthorizationError(
        `User ${context.user.id} unauthorized for tool ${toolName}`,
        `Anda tidak memiliki hak akses untuk menggunakan fitur ${toolName}.`
      );
    }
      ...
      try {
        await prisma.toolExecution.create({
          data: {
            ...
            toolName,
            inputParams: rawParams as any,
            outputResult: outputResult ? (outputResult as any) : null,
            status,
            durationMs,

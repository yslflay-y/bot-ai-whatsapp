...
export class AccessControlService {
    ...
    adminJids;
    constructor() {
        this.allowedJids = new Set(config.ALLOWLIST_NUMBERS.map((n) => normalizePhoneNumber(n).jid));
        this.adminJids = new Set(config.ADMIN_NUMBERS.map((n) => normalizePhoneNumber(n).jid));
    }
    /**
    ...
    async authorize(senderJid, pushName) {
        ...
        const assignedRole = isAdmin ? Role.ADMIN : Role.USER;
    ...
    requireAdmin(user) {
        if (user.role !== Role.ADMIN) {
            throw new AuthorizationError(`User ${user.id} does not have ADMIN privileges`, "Perintah ini hanya dapat dijalankan oleh Administrator.");
        }
    }
}

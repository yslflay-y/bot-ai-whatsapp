...
export class MessagePipeline {
    ...
    async handleInbound(message) {
        ...
        logger.info({ from: message.fromJid, type: message.type, id: message.id }, "Inbound message received in pipeline");
        ...
        if (!rateCheck.allowed) {
            ...
            if (whatsapp) {
                await whatsapp.sendMessage(user.jid, "⚠️ Mohon maaf, Anda mengirim pesan terlalu cepat. Mohon tunggu sebentar sebelum mengirim pesan baru.");
            }
            return;
        }
        ...
        try {
            ...
            logger.info({ userId: user.id, durationMs: Date.now() - start, tokens: response.tokenCount }, "Processed inbound message successfully");
        }
        catch (err) {
            logger.error({ userId: user.id, err }, "Error generating AI response for message");
            if (whatsapp) {
                await whatsapp.sendMessage(user.jid, "Maaf, terjadi kendala saat memproses permintaan Anda. Mohon coba sesaat lagi.");
            }
        }
    }
    /**
     ...
     */
    async handleSlashCommand(command, user) {
        const parts = command.split(" ");
        const cmd = parts[0]?.toLowerCase();
        const arg = parts.slice(1).join(" ").trim();
        switch (cmd) {
            case "/help":
                return (`🤖 *BANTUAN ASISTEN AI:*\n\n` +
            ...
            case "/note":
                if (!arg)
                    return "Gunakan format: /note [isi catatan Anda]";
            ...
            case "/notes": {
                ...
                if (notes.length === 0)
                    return "Anda belum memiliki catatan tersimpan.";
                return (`📝 *CATATAN TERBARU ANDA:*\n\n` +
